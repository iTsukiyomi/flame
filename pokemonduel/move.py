...
class Move():
    ...
    def attack(self, attacker, defender, battle):
        ...
        for hit in range(hits):
            ...
            if self.effect == 416:
                ignore_stages = defender.ability(attacker=attacker, move=self) == Ability.UNAWARE
                a = max(attacker.get_attack(battle, critical=critical, ignore_stages=ignore_stages),
                        attacker.get_spatk(battle, critical=critical, ignore_stages=ignore_stages))

            if attacker.flash_fire and current_type == ElementType.FIRE:
                a *= 1.5
            if defender.ability(attacker=attacker, move=self) == Ability.THICK_FAT and current_type in (
            ...
            if current_type == ElementType.WATER and battle.weather.get() in ("rain", "h-rain"):
                ...
                damage *= 0.5
            elif current_type == ElementType.FIRE and battle.weather.get() in ("sun", "h-sun"):
                damage *= 1.5
            elif current_type == ElementType.WATER and battle.weather.get() in ("sun", "h-sun"):
                damage *= 0.5
            ...
            if defender.ability(attacker=attacker, move=self) in (Ability.FILTER, Ability.PRISM_ARMOR,
                                                                  Ability.SOLID_ROCK) and effectiveness > 1:
                damage *= .75
            if attacker.ability() == Ability.NEUROFORCE and effectiveness > 1:
                damage *= 1.25
            if defender.ability(attacker=attacker,
                                move=self) == Ability.ICE_SCALES and damage_class == DamageClass.SPECIAL:
                damage *= .5
            ...
            if defender.ability(attacker=attacker, move=self) == Ability.PUNK_ROCK and self.is_sound_based():
                damage *= .5
            if defender.ability(attacker=attacker,
                                move=self) == Ability.HEATPROOF and current_type == ElementType.FIRE:
                damage *= .5
            if defender.ability(attacker=attacker,
                                move=self) == Ability.PURIFYING_SALT and current_type == ElementType.GHOST:
                damage *= .5
            if Ability.DARK_AURA in (attacker.ability(), defender.ability(attacker=attacker,
                                                                          move=self)) and current_type == ElementType.DARK:
                if Ability.AURA_BREAK in (attacker.ability(), defender.ability(attacker=attacker, move=self)):
                    damage *= .75
                else:
                    damage *= 4 / 3
            if Ability.FAIRY_AURA in (attacker.ability(), defender.ability(attacker=attacker,
                                                                           move=self)) and current_type == ElementType.FAIRY:
                if Ability.AURA_BREAK in (attacker.ability(), defender.ability(attacker=attacker, move=self)):
                    damage *= .75
                else:
                    damage *= 4 / 3
            if defender.ability(attacker=attacker,
                                move=self) == Ability.DRY_SKIN and current_type == ElementType.FIRE:
                damage *= 1.25
            ...
            if defender.ability(attacker=attacker, move=self) in (Ability.MULTISCALE,
                                                                  Ability.SHADOW_SHIELD) and defender.hp == defender.starting_hp:
                damage *= .5
            ...
            # Do the damage
            msgadd, damage = defender._damage(damage, battle, move=self, move_type=current_type, attacker=attacker,
                                              critical=critical, drain_heal_ratio=drain_heal_ratio)
            msg += msgadd
    ...
    def get_power(self, attacker, defender, battle):
        ...
        if attacker.ability() == Ability.WATER_BUBBLE and current_type == ElementType.WATER:
            power *= 2
        if defender.ability(attacker=attacker,
                            move=self) == Ability.WATER_BUBBLE and current_type == ElementType.FIRE:
            power *= .5
        ...
        if attacker.ability() == Ability.NORMALIZE and current_type == ElementType.NORMAL:
            power *= 1.2
        if attacker.ability() == Ability.SAND_FORCE and current_type in (ElementType.ROCK, ElementType.GROUND,
                                                                         ElementType.STEEL) and battle.weather.get() == "sandstorm":
            power *= 1.3
        ...
        if attacker.held_item == "zap-plate" and current_type == ElementType.ELECTRIC:
            power *= 1.2
        if attacker.held_item == "adamant-orb" and current_type in (ElementType.DRAGON,
                                                                    ElementType.STEEL) and attacker._name == "Dialga":
            power *= 1.2
        if attacker.held_item == "griseous-orb" and current_type in (ElementType.DRAGON,
                                                                     ElementType.GHOST) and attacker._name == "Giratina":
            power *= 1.2
        if attacker.held_item == "soul-dew" and current_type in (ElementType.DRAGON,
                                                                 ElementType.PSYCHIC) and attacker._name in (
                "Latios", "Latias"):
            power *= 1.2
        if attacker.held_item == "lustrous-orb" and current_type in (ElementType.DRAGON,
                                                                     ElementType.WATER) and attacker._name == "Palkia":
            power *= 1.2
        ...
        if self.effect == 204 and battle.weather.get() in ("hail", "snow", "sandstorm", "rain", "h-rain", "sun",
                                                           "h-sun"):
            power *= 2
        # During hail/snow, rain-dance, or sandstorm, power is halved.
        if self.effect == 152 and battle.weather.get() in ("rain", "hail", "snow"):
            power *= 0.5
        ...
        if self.effect == 129 and (
                isinstance(defender.owner.selected_action, int) or defender.owner.selected_action.effect in (128,
                                                                                                             154,
                                                                                                             229,
                                                                                                             347,
                                                                                                             493)):
        ...
        if attacker.owner.selected_action is not None and not isinstance(attacker.owner.selected_action,
                                                                         int) and attacker.owner.selected_action.effect == 242:
            power *= 1.5
        ...
        if self.effect == 443 and battle.terrain.item == "electric" and defender.grounded(battle, attacker=attacker,
                                                                                          move=self):
            power *= 2
        ...
        if battle.terrain.item == "grassy" and attacker.grounded(battle) and current_type == ElementType.GRASS:
            power *= 1.3
        if battle.terrain.item == "grassy" and defender.grounded(battle, attacker=attacker,
                                                                 move=self) and self.id in (89, 222, 523):
            power *= 0.5
        if battle.terrain.item == "electric" and attacker.grounded(battle) and current_type == ElementType.ELECTRIC:
            power *= 1.3
        if battle.terrain.item == "misty" and defender.grounded(battle, attacker=attacker,
                                                                move=self) and current_type == ElementType.DRAGON:
            power *= 0.5
        ...
        if attacker.charge.active() and current_type == ElementType.ELECTRIC:
            power *= 2
        if (
                attacker.owner.mud_sport.active() or defender.owner.mud_sport.active()) and current_type == ElementType.ELECTRIC:
            power //= 3
        if (
                attacker.owner.water_sport.active() or defender.owner.water_sport.active()) and current_type == ElementType.FIRE:
            power //= 3
    ...
    def get_type(self, attacker, defender, battle):
        ...
        if attacker.electrify:
            return ElementType.ELECTRIC
        if self.effect == 204:
            if battle.weather.get() in ("hail", "snow"):
                return ElementType.ICE
    ...
    def check_executable(self, attacker, defender, battle):
        ...
        if self.is_powder_or_spore() and (
                ElementType.GRASS in defender.type_ids or defender.ability(attacker=attacker,
                                                                           move=self) == Ability.OVERCOAT or defender.held_item == "safety-goggles"):
            return False
        if battle.weather.get() == "h-sun" and self.get_type(attacker, defender,
                                                             battle) == ElementType.WATER and self.damage_class != DamageClass.STATUS:
            return False
        if battle.weather.get() == "h-rain" and self.get_type(attacker, defender,
                                                              battle) == ElementType.FIRE and self.damage_class != DamageClass.STATUS:
            return False
        ...
        if self.effect == 91 and (
                defender.encore.active() or defender.last_move is None or defender.last_move.pp == 0):
            return False
        if self.effect == 87 and (
                defender.disable.active() or defender.last_move is None or defender.last_move.pp == 0):
            return False
        ...
        if self.effect in (10, 243) and (
                defender.last_move is None or not defender.last_move.selectable_by_mirror_move()):
            return False
        ...
        if self.effect == 235 and (not attacker.nv.current or defender.nv.current):
            return False
        if self.effect in (121, 266) and ("-x" in (attacker.gender,
                                                   defender.gender) or attacker.gender == defender.gender or defender.ability(
            attacker=attacker, move=self) == Ability.OBLIVIOUS):
            return False
        ...
        if self.effect == 98 and not any(m.selectable_by_sleep_talk() for m in attacker.moves):
            return False
        # Gen 9 - Aurora Veil now works in snow as well as hail
        if self.effect == 407 and not battle.weather.get() in ("hail", "snow"):
            return False
        ...
        if self.effect == 248 and defender.ability(attacker=attacker, move=self) == Ability.INSOMNIA:
            return False
        if self.effect in (242, 249) and (defender.has_moved or isinstance(defender.owner.selected_action,
                                                                           int) or defender.owner.selected_action.damage_class == DamageClass.STATUS):
            return False
        ...
        if self.effect == 181 and attacker.get_assist_move() is None:
            return False
        if self.effect in (112, 117, 184, 195, 196, 279, 307, 345, 350, 354, 356, 362, 378, 384, 454, 488,
                           499) and defender.has_moved:
            return False
        if self.effect == 192 and not (
                attacker.ability_changeable() and attacker.ability_giveable() and defender.ability_changeable() and defender.ability_giveable()):
            return False
        ...
        if self.effect == 234 and (not attacker.held_item.power or attacker.ability() == Ability.STICKY_HOLD):
            return False
        if self.effect == 178 and (Ability.STICKY_HOLD in (attacker.ability(), defender.ability(attacker=attacker,
                                                                                                move=self)) or not attacker.held_item.can_remove() or not defender.held_item.can_remove()):
            return False
        ...
        if self.effect == 188 and (
                defender.nv.current or defender.ability(attacker=attacker, move=self) in (Ability.INSOMNIA,
                                                                                          Ability.VITAL_SPIRIT,
                                                                                          Ability.SWEET_VEIL) or defender.yawn.active()):
            return False
        if self.effect == 188 and battle.terrain.item == "electric" and attacker.grounded(battle):
            return False
        if self.effect in (340, 351) and not any(ElementType.GRASS in p.type_ids and p.grounded(
                battle) and not p.dive and not p.dig and not p.fly and not p.shadow_force for p in
                                                 (attacker, defender)):
            return False
        if self.effect == 341 and defender.owner.sticky_web:
            return False
        if self.effect in (112, 117, 356, 362, 384, 454, 488, 499) and random.randint(1,
                                                                                      attacker.protection_chance) != 1:
            return False
        if self.effect == 403 and (
                defender.last_move is None or defender.last_move.pp == 0 or not defender.last_move.selectable_by_instruct() or defender.locked_move is not None):
            return False
        if self.effect == 378 and (ElementType.GRASS in defender.type_ids or defender.ability(attacker=attacker,
                                                                                              move=self) == Ability.OVERCOAT or defender.held_item == "safety-goggles"):
            return False
        if self.effect == 233 and defender.embargo.active():
            return False
        if self.effect == 324 and (
                not attacker.held_item.has_item() or defender.held_item.has_item() or not attacker.held_item.can_remove()):
            return False
        if self.effect == 185 and (attacker.held_item.has_item() or attacker.held_item.last_used is None):
            return False
        if self.effect == 430 and (
                not defender.held_item.has_item() or not defender.held_item.can_remove() or defender.corrosive_gas):
            return False
        ...
        if self.effect == 38 and (
                attacker.nv.sleep() or attacker.hp == attacker.starting_hp or attacker._name == "Minior"):
            return False
        if self.effect == 427 and attacker.no_retreat:
            return False
        if self.effect == 99 and attacker.destiny_bond_cooldown.active():
            return False
        if self.effect in (116, 137, 138, 165, 510) and battle.weather.get() in ("h-rain", "h-sun", "h-wind"):
            return False
        if self.effect in (8, 420, 444) and Ability.DAMP in (attacker.ability(),
        ...
        if self.effect == 280 and any(
                [defender.defense_split, defender.spdef_split, attacker.defense_split, attacker.spdef_split]):
            return False
        if self.effect == 281 and any(
                [defender.attack_split, defender.spatk_split, attacker.attack_split, attacker.spatk_split]):
            return False
        if self.effect == 456 and (defender.type_ids == [ElementType.PSYCHIC] or defender.ability(attacker=attacker,
                                                                                                  move=self) == Ability.RKS_SYSTEM):
            return False
        if self.effect == 83 and self not in attacker.moves:
            return False
        if self.effect == 501 and (defender.has_moved or isinstance(defender.owner.selected_action,
                                                                    int) or defender.owner.selected_action.get_priority(
            defender, attacker, battle) <= 0):
            return False
        if defender.ability(attacker=attacker, move=self) in (Ability.QUEENLY_MAJESTY, Ability.DAZZLING,
                                                              Ability.ARMOR_TAIL) and self.get_priority(attacker,
                                                                                                        defender,
                                                                                                        battle) > 0:
            return False
        return True
    ...
    def check_protect(self, attacker, defender, battle):
        ...
        if defender.baneful_bunker:
            if self.makes_contact(attacker) and attacker.held_item != "protective-pads":
                msg += attacker.nv.apply_status("poison", battle, attacker=defender,
                                                source=f"{defender.name}'s baneful bunker")
            return False, msg
        if defender.wide_guard and self.targets_multiple():
            return False, msg
        if self.get_priority(attacker, defender,
                             battle) > 0 and battle.terrain.item == "psychic" and defender.grounded(battle,
                                                                                                    attacker=attacker,
                                                                                                    move=self):
            return False, msg
        ...
        if defender.burning_bulwark and self.damage_class != DamageClass.STATUS:
            if self.makes_contact(attacker) and attacker.held_item != "protective-pads":
                msg += attacker.nv.apply_status("burn", battle, attacker=defender,
    ...
    def check_hit(self, attacker, defender, battle):
        ...
        # During hail/snow, this bypasses accuracy checks
        if self.effect == 261 and battle.weather.get() in ("hail", "snow"):
            return True
        ...
        if self.targets_opponent():
            if defender.ability(attacker=attacker,
                                move=self) == Ability.WONDER_SKIN and self.damage_class == DamageClass.STATUS:
                accuracy = 50
        ...
        if self.targets_opponent():
            if defender.ability(attacker=attacker,
                                move=self) == Ability.TANGLED_FEET and defender.confusion.active():
                accuracy *= .5
            if defender.ability(attacker=attacker,
                                move=self) == Ability.SAND_VEIL and battle.weather.get() == "sandstorm":
                accuracy *= .8
            # Gen 9 - Snow Cloak now works in snow as well as hail
            if defender.ability(attacker=attacker, move=self) == Ability.SNOW_CLOAK and battle.weather.get() in (
                    "hail", "snow"):
                accuracy *= .8
    ...
    def check_effective(self, attacker, defender, battle):
        ...
        if attacker.ability() == Ability.PRANKSTER and ElementType.DARK in defender.type_ids:
            ...
            if not isinstance(attacker.owner.selected_action,
                              int) and attacker.owner.selected_action.damage_class == DamageClass.STATUS:
                return False
        if defender.ability(attacker=attacker,
                            move=self) == Ability.GOOD_AS_GOLD and self.damage_class == DamageClass.STATUS:
            return False
        ...
        if current_type == ElementType.GROUND and not defender.grounded(battle, attacker=attacker,
                                                                        move=self) and self.effect != 373 and not battle.inverse_battle:
            return False
        if self.effect != 459:
            if current_type == ElementType.ELECTRIC and defender.ability(attacker=attacker,
                                                                         move=self) == Ability.VOLT_ABSORB and defender.hp == defender.starting_hp:
                return False
            if current_type == ElementType.WATER and defender.ability(attacker=attacker, move=self) in (
                    Ability.WATER_ABSORB, Ability.DRY_SKIN) and defender.hp == defender.starting_hp:
                return False
        if current_type == ElementType.FIRE and defender.ability(attacker=attacker,
                                                                 move=self) == Ability.FLASH_FIRE and defender.flash_fire:
            return False
    ...
    def setup(self, attacker, defender, battle):
        ...
        msg = ""
        if self.effect == 129 and (
                isinstance(defender.owner.selected_action, int) or defender.owner.selected_action.effect in (128, 154,
                                                                                                             229, 347,
                                                                                                             493)):
    ...
    def use(self, attacker, defender, battle, *, use_pp=True, override_sleep=False, bounced=False):
        ...
        # Powder damage
        if attacker.powdered and current_type == ElementType.FIRE and battle.weather.get() not in ("h-rain", "rain"):
        ...
        # Turn 2 hit moves
        elif self.effect in (40, 76, 146, 152, 156, 256, 257, 264, 273, 332, 333, 366, 451,
                             502) and attacker.locked_move:
        ...
        if self.is_affected_by_magic_coat() and (defender.ability(attacker=attacker,
                                                                  move=self) == Ability.MAGIC_BOUNCE or defender.magic_coat) and not bounced:
        ...
        if self.targets_opponent() and self.effect != 459:
            # Heal
            if current_type == ElementType.ELECTRIC and defender.ability(attacker=attacker,
                                                                         move=self) == Ability.VOLT_ABSORB:
                msg += f"{defender.name}'s volt absorb absorbed the move!\n"
                msg += defender.heal(defender.starting_hp // 4, source="absorbing the move")
                return msg
            if current_type == ElementType.WATER and defender.ability(attacker=attacker,
                                                                      move=self) == Ability.WATER_ABSORB:
        ...
        # Counter attack moves
            ...
            msg += defender.damage(int(1.5 * attacker.last_move_damage[0]), battle, move=self, move_type=current_type,
                                   attacker=attacker)
            numhits = 1
        elif self.effect == 145:
            msg += defender.damage(2 * attacker.last_move_damage[0], battle, move=self, move_type=current_type,
                                   attacker=attacker)
            numhits = 1
        elif self.effect == 90:
            msg += defender.damage(2 * attacker.last_move_damage[0], battle, move=self, move_type=current_type,
        ...
        # Static-damage moves
            ...
            msg += defender.damage(int(attacker.level * scale), battle, move=self, move_type=current_type,
                                   attacker=attacker)
            numhits = 1
            ...
            msg += defender.damage(max(0, defender.hp - attacker.hp), battle, move=self, move_type=current_type,
                                   attacker=attacker)
            numhits = 1
        elif self.effect == 39:
            ...
            numhits = 1
        # Gen 9 - Ruination effect
        elif self.effect == 504:
            msg += defender.damage(defender.hp // 2, battle, move=self, move_type=current_type, attacker=attacker)
            numhits = 1
        ...
        if self.effect == 163:
            msg += attacker.heal(attacker.starting_hp // {1: 4, 2: 2, 3: 1}[attacker.stockpile],
        ...
        # Stage changes
        # +1
        if self.effect in (11, 209, 213, 278, 313, 323, 328, 392, 414, 427, 468, 472, 487, 505, 506):
        ...
        if self.effect in (130, 213, 291, 296, 414, 427, 442, 468, 469, 487, 506):
        ...
        if self.effect == 359:
            if random.randint(1, 100) <= effect_chance:
                msg += attacker.append_defense(2, attacker=attacker, move=self)
        # -1
        if self.effect in (19, 206, 344, 347, 357, 365, 388, 412, 507):
        ...
        if self.effect in (344, 347, 358, 412, 508):
            msg += defender.append_spatk(-1, attacker=attacker, move=self)
        if self.effect == 428:
            msg += defender.append_spdef(-1, attacker=attacker, move=self)
        if self.effect in (331, 390, 509):
        ...
        if self.effect in (219, 335):
            msg += attacker.append_speed(-1, attacker=attacker, move=self)
        # Gen 9 - Make It Rain effect (post-damage SpA drop)
        if self.effect == 508:
            msg += attacker.append_spatk(-1, attacker=attacker, move=self)
        # -2
        ...
        # Gen 9 - Snowscape weather
        if self.effect == 510:
            msg += battle.weather.set("snow", attacker)
        ...
        # Syrup Bomb
            ...
            # Gen 9 - Salt Cure ongoing effect
            if self.effect == 511:
                defender.salt_cure = True
                msg += f"{defender.name} is being cured with salt!\n"

            # Gen 9 - Comeuppance counter move
            if self.effect == 512:
                damage_taken = sum(attacker.last_move_damage or [0])
                if damage_taken > 0:
                    msg += defender.damage(int(damage_taken * 1.5), battle, move=self, move_type=current_type,
                                           attacker=attacker)
                    numhits = 1
                else:
                    msg += "But it failed!\n"
